...
bool FunctionAliasPass::runOnModule(Module &M) {
  ...
  for (const auto &pair : FunctionAlias) {
    ...
    if (lastColon == std::string::npos) {
      linker_error("function-alias: no replacement given");
    }
    std::string pattern = pair.substr(0, lastColon);
    std::string replacement = pair.substr(lastColon + 1);

    if (pattern.empty())
      linker_error("function-alias: name or pattern cannot be empty");
    if (replacement.empty())
      linker_error("function-alias: replacement cannot be empty");

    if (pattern == replacement) {
      linker_error("function-alias: @%s cannot replace itself", pattern.c_str());
    }
    ...
    if (!isFunctionOrGlobalFunctionAlias(replacementValue))
      linker_error("function-alias: replacement function @%s could not be found",
                 replacement.c_str());
    ...
    if (isFunctionOrGlobalFunctionAlias(match)) {
      if (tryToReplace(match, replacementValue)) {
        modified = true;
        linker_message("function-alias: replaced @%s with @%s", pattern.c_str(),
                     replacement.c_str());
        continue;
      }
    }
    if (match != nullptr) {
      ...
      linker_error("function-alias: no (replacable) match for '%s' found",
                 pattern.c_str());
    }
    ...
    if (!regex.isValid(error)) {
      linker_error("function-alias: '%s' is not a valid regex: %s",
    ...
    for (GlobalValue &global : M.global_values()) {
      ...
      if (match && matchVec[0].str() == global.getName().str()) {
        if (global.getName().str() == replacement) {
          linker_message("function-alias: do not replace @%s (matching '%s') "
    ...
    for (GlobalValue *global : matches) {
      ...
      if (tryToReplace(global, replacementValue)) {
        modified = true;
        matchFound = true;
        linker_message("function-alias: replaced @%s (matching '%s') with @%s",
...
bool FunctionAliasPass::checkType(const GlobalValue *match,
  ...
  if (MFT->getReturnType() != RFT->getReturnType()) {
    linker_message("function-alias: @%s could not be replaced with @%s: "
                 "return type differs",
